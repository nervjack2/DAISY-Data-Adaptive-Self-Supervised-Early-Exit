...
from s3prl.upstream.interfaces import Featurizer, FeaturizerDynamic
...
class Runner():
    ...
    def _get_featurizer(self):
        if self.args.featurizer_type == 'normal':
            model = Featurizer(
        ...
        elif self.args.featurizer_type in ['dynamic', 'dynamic-s']: 
            model = FeaturizerDynamic(
                upstream = self.upstream.model,
                feature_selection = self.args.upstream_feature_selection,
                layer_selection = self.args.upstream_layer_selection,
                upstream_device = self.args.device,
                normalize = self.args.upstream_feature_normalize,
                featurizer_type = self.args.featurizer_type,
            ).to(self.args.device)
        else:
            print(f"Do not support featurizer type {self.args.featurizer_type}")
            exit(0)
    ...
    def train(self):
        ...
        if self.args.upstream_log:
            u_fp = open(self.args.upstream_log, 'w') 
            ...
            for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc='train', file=tqdm_file)):
                ...
                try:
                    ...
                    with torch.cuda.amp.autocast(enabled=amp):
                        # if 'ivector' not in self.args.upstream:
                        if self.upstream.trainable:
                            features = self.upstream.model(wavs, mode='train')
                        else:
                            with torch.no_grad():
                                features = self.upstream.model(wavs, mode='train')
                        # else: 
                        #     if self.upstream.trainable:
                        #         features = self.upstream.model(wavs, others[1])
                        #     else:
                        #         with torch.no_grad():
                        #             features = self.upstream.model(wavs, others[1])

                        features, num_feat = self.featurizer.model(wavs, features)
    
                        if specaug:
                            features, _ = specaug(features)
            
                        # if self.args.downstream != 'speaker_linear_utter_libri':
                ...
                if global_step % self.config['runner']['log_step'] == 0:
                    # if self.args.upstream_log:
                        # u_fp.write(f"Steps {global_step}:\n"+
                        #         f"Avg layer = {self.upstream.model.n_layer / self.upstream.model.n_sample}\n"+
                        #         f"Min layer = {self.upstream.model.min_layer}\n"+
                        #         f"Max layer= {self.upstream.model.max_layer}\n")
                        # u_fp.flush()
        ...
        if self.args.upstream_log:
            u_fp.write(f"Training time = {total_train_time}\n")
        pbar.close()
        ...
        if self.args.upstream_log:
            total_layer = sum([l*(i+1) for i, l in enumerate(self.upstream.model.n_layer)])
            u_fp.write(f"Steps {global_step}:\n"+
                                f"Avg layer = {total_layer / self.upstream.model.n_sample}\n"+
                                f"Min layer = {self.upstream.model.min_layer}\n"+
                                f"Max layer= {self.upstream.model.max_layer}\n")

            hyper_param = self.upstream.model.hyper_param
            hyper_param['n_sample'] = self.upstream.model.n_sample
            hyper_param['n_layer'] = self.upstream.model.n_layer
            hyper_param['min_layer'] = self.upstream.model.min_layer
            hyper_param['max_layer'] = self.upstream.model.max_layer

            with open(self.upstream.model.model_config, 'w') as file:
                import yaml
                hyper_param = yaml.dump(hyper_param, file, default_flow_style=False)
    ...
    def evaluate(self, split=None, logger=None, global_step=0):
        ...
        for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc=split, total=evaluate_steps)):
            wav_len += sum([len(wav) for wav in wavs])
            if batch_id > evaluate_steps:
                break

            wavs = [torch.FloatTensor(wav).to(self.args.device) for wav in wavs]
            with torch.no_grad():
                features = self.upstream.model(wavs, mode='eval')
                ...
                batch_ids.append(batch_id)
        end_x = time.time()
        print(f"Inference time = {end_x - start_x}")
        print(f"Total wav length = {wav_len/16000}")
        print(f"Real time factor = {(wav_len/16000)/(end_x - start_x)}")
