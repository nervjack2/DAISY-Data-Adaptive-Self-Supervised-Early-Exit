...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, padding_mask=None, final_proj=None, cls_pred_pos='after-norm', u_threshold=0.35, mode='train', ext_range=None):
        x, layer_results = self.extract_features(x, padding_mask, final_proj, cls_pred_pos, u_threshold, mode, ext_range)
    ...
    def extract_features(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                ...
                if mode == 'train' or (mode == 'eval' and (i+1 in ext_range)):
                    if final_proj != None:
                        if cls_pred_pos == 'before-norm':
                            frame_pred = lr.permute(1,0,2)
                            d = frame_pred.shape[2]
                            frame_pred = frame_pred.reshape(-1,d)
                        else:
                            frame_pred = x.permute(1,0,2)
                            d = frame_pred.shape[2]
                            frame_pred = frame_pred.reshape(-1,d)
                    
                        u_score = F.softmax(final_proj[i](frame_pred), dim=-1)
                        u_score = u_score[~padding_mask.reshape(-1)]
                        u_cls = torch.mean(-torch.sum(torch.log(u_score) * u_score, dim=-1)) # Entropy
                        if u_cls.item() <= u_threshold:
