...
class Featurizer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            if not hasattr(upstream, 'name') or 'ivector' not in upstream.name:
                paired_features = upstream(paired_wavs)
            else:
                paired_features = upstream(paired_wavs, ['' for i in range(len(paired_wavs))])
    ...
    def forward(
        ...
        return self.tolist(paired_wavs, feature), feat_len

class FeaturizerDynamic(nn.Module):
    def __init__(
        self,
        upstream: UpstreamBase,
        feature_selection: str = "hidden_states",
        ...
        **kwargs,
        ...
        with torch.no_grad():
            # if not hasattr(upstream, 'name') or 'ivector' not in upstream.name:
            paired_features = upstream(paired_wavs, mode='eval')
            # else:
            #     paired_features = upstream(paired_wavs, ['' for i in range(len(paired_wavs))])
        ...
        feature = self._select_feature(paired_features)
        if isinstance(feature, (list, tuple)):
            self.max_layer_num = 13 #todo: make it flexible to different models
            show(
                f"[{self.name}] - Take maximum of {self.max_layer_num} features and weighted sum them.",
                file=sys.stderr,
            )
            if self.featurizer_type == 'dynamic':
                self.weights = nn.Parameter(torch.zeros(self.max_layer_num))
            elif self.featurizer_type == 'dynamic-s':
                self.weights = nn.Parameter(torch.zeros((self.max_layer_num, self.max_layer_num)))

            feat_len = len(feature)
            feature = self._weighted_sum([f.cpu() for f in feature], feat_len)
        else:
            feature = feature.cpu()
    ...
    def _weighted_sum(self, feature, feat_len):
        assert feat_len == len(feature)
        stacked_feature = torch.stack(feature, dim=0)
        ...
        _, *origin_shape = stacked_feature.shape
        stacked_feature = stacked_feature.view(feat_len, -1)
        if self.featurizer_type == 'dynamic':
            norm_weights = F.softmax(self.weights[:feat_len], dim=-1)
        elif self.featurizer_type == 'dynamic-s':
            norm_weights = F.softmax(self.weights[feat_len-1,:feat_len], dim=-1)
        weighted_feature = (norm_weights.unsqueeze(-1) * stacked_feature).sum(dim=0)
        weighted_feature = weighted_feature.view(*origin_shape)
    ...
    def forward(
        ...
        if isinstance(feature, (list, tuple)):
            feat_len = len(feature)
            feature = self._weighted_sum(feature, feat_len)
